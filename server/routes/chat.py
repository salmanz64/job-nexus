...
def get_messages(user1: str, user2: str, db: Session = Depends(get_db)):
    ...
    profile1 = db.query(Profile).filter(Profile.user_id == user1).first()
    if not profile1:
        raise HTTPException(status_code=404, detail="Profile1 Not Found")

    profile2 = db.query(Profile).filter(Profile.user_id == user2).first()
    if not profile2:
        raise HTTPException(status_code=404, detail="Profile2 Not Found")

    p1 = profile1.id
    p2 = profile2.id

    # Mark messages sent to p1 as read
    (
        db.query(Message)
        .filter(
            Message.sender_id == p2,
            Message.receiver_id == p1,
            Message.is_read == False
        )
        .update({Message.is_read: True})
    )
    ...
    # Fetch conversation using PROFILE IDs
    messages = (
        db.query(Message)
        .filter(
